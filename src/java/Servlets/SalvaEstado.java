 ...
 * @author Felipe
 */
@WebServlet(name = "SalvaEstado", urlPatterns = {"/SalvaEstado"})
public class SalvaEstado extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String nome, sigla, AchaEstado;
//        AchaEstado = "Nenhum";
        nome = request.getParameter("txtEstado");
        ...
        if((!nome.equals("")) && !sigla.equals(""))
        {
            try{
                ...
                if(resp != null){
                    conexao.ExecutaSql("select * from estados where nome_estado = '"+nome+"' and sigla_estado = '"+sigla+"'");

                    if(!conexao.resultSet.first())
                    {
                        PreparedStatement pst = resp.prepareStatement("INSERT INTO estados (nome_estado, sigla_estado) values (?, ?)");
